...
int main()
{
    ...
    for(int i=1;i<=k;i++)
    {// i is maximum
        double x=1.00,y=1.00;// (i/k)^n-(i-1/k)^n 
        //  every child select 1/2/3/..i number of candy
        // then remove the cases when i number isn't chosen by anyone
        for(int j=1;j<=n;j++)
        {
            x=x*((double)i/(double)k);
            y=y*((double)(i-1)/(double)k);
            
        }
        ans=ans+(x-y)*(double)i;
    }
