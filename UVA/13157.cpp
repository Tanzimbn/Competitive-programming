...
void solve()
{
   ...
   {
      ans=(ans+(n-prime[i])*(m-1))%mod;
   }
   for(int i=1;i<prime.size()&&prime[i]+3<=n;i++)
   {
     if(prime[i]+2!=prime[i+1]) continue;
     ll temp=(((m-1)*(m-2))/2)%mod;
     ans=(ans+2*((n-prime[i]-2)*temp)%mod)%mod;
   }
   for(int i=1;i<prime.size()&&prime[i]+5<=n;i++)
   {
     if(prime[i]+2!=prime[i+1]||prime[i]+4!=prime[i+2]) continue;
     ll temp=(((m-1)*(m-2)*(m-3))/6)%mod;
     ans=(ans+((n-prime[i]-4)*temp)%mod)%mod;
   }
   cout<<ans<<endl;
} 
int main()
