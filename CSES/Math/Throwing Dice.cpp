...
void check(ll (*a)[7],ll b[][7])
{
    ...
    for(int i=0;i<6;i++)
    {
        for(int j=0;j<6;j++) a[i][j]=ans[i][j];
    }
}
int main()
{
    ...
    ll n;
    cin>>n;
    ll a[]={32,16,8,4,2,1};                    // Matrix Exponentiation
    if(n<7) {cout<<a[5-n+1]<<endl;return 0;}
    ll t[7][7];
    for(int i=0;i<6;i++) t[0][i]=1;
    for(int i=1;i<6;i++)
    {
        for(int j=0;j<6;j++)
        {
