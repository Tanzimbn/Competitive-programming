...
void pri()
{
   int dif=prime[1]-prime[0],s=0;
   for(int i=2;i<prime.size();i++)
   {
      if(prime[i]-prime[i-1]!=dif)
      {
         if(i-s>=3)
         {
            vs.PB(prime[s]);
         ...
         s=i-1;dif=prime[i]-prime[i-1];
      }
   }
   if(prime.size()-s>=3)
   {
      vs.PB(prime[s]);
      ve.PB(prime[prime.size()-1]);
      vd.PB(dif);
   }
   return ;
}
void solve(int x, int y)
{
    ...
    for(;vs[i]<=y&&i<vs.size();i++)
    {
      if(ve[i]>y) continue;
      cout<<vs[i];
      for(int j=vs[i]+vd[i];j<=ve[i];j+=vd[i]) cout<<" "<<j;
      cout<<endl;
    }
}
int main()
