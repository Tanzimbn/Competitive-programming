...
int main()
{
    ...
    while(cin>>n>>kk)
    {
        ...
        string s;
        for(int i=0;i<100;i++)
            for(int j=0;j<100;j++) if(i!=j) dp[i][j]=INF;
        cin.ignore();
        for(int al=0;al<n;al++)
        {
            ...
            for(int i=0;i<num;i++)
            {
                for(int j=0;j<i;j++)
                {
                    dp[a[i]][a[j]]=dp[a[j]][a[i]]=min(dp[a[i]][a[j]],(a[i]-a[j])*t[al]);
                }
            }
        }
        for(int i=0;i<100;i++)
        {
            for(int j=0;j<100;j++)
            {
                for(int k=0;k<100;k++)
                {
                    dp[j][k]=min(dp[j][k],dp[j][i]+dp[i][k]+60);
                }
            }
        }
        if(dp[0][kk]==INF) cout<<"IMPOSSIBLE\n";
        else cout<<dp[0][kk]<<endl;
    }
}
