...
{
    vector<vl> ans (n+5,vl(n+5,INFLL) );
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n;j++)
        {
...
vector<vl> check(vector<vl> x,ll pw,ll n)
{
    if(pw==1) return x;
    vector<vl> v=check(x,pw/2,n);
    v=matrix_multi(v,v,n);
    if(pw%2==1) v=matrix_multi(v,x,n);
    return v;
}
int main()
{
    ...
    ll n,m,k;
    cin>>n>>m>>k;
    vector<vl> v(n+5,vl(n+5,INFLL)),ans(n+5,vl(n+5));
    while(m--)
    {
        ll x,y,c;
        cin>>x>>y>>c;
    ...
    if(ans[0][n-1]==INFLL) cout<<-1<<endl;
    else
    cout<<ans[0][n-1]<<endl;
 }
